...
import { useBookingUserEmails } from "@/features/dashboard/model/use-booking-user-emails"
import { hasBookingInMonth } from "@/features/dashboard/lib/bookings"
import { computeSeasonalityMultipliers } from "@/features/dashboard/lib/seasonality"


/**
 * Dashboard (Owner): timeline/calendário + estatísticas + gráficos.
 *
 * Responsabilidades desta view:
 * - carregar propriedades e reservas do utilizador
 * - aplicar filtros/ordenação e foco (uma propriedade)
 * - calcular métricas e datasets para charts
 * - suportar hover rico no calendário (cache de emails por userId)
 * - carregar e projetar multiplicadores de sazonalidade quando há foco
 */
export function DashBoardView() {
  ...
  /**
   * Quando definido, a dashboard só mostra dados desta propriedade.
   */
  const [focusPropertie, setFocusPropertie] = useState<PropertyListItem | undefined>(
    undefined
  )
  ...
  /**
   * Data (ano e mês) que a dashboard está a analisar.
   */
  ...
  /**
   * Cache: propriedadeId -> tem reservas no mês em foco?
   *
   * Usado para:
   * - filtro "Só com reservas"
   * - ordenação (as com reservas aparecem primeiro)
   */
  ...
  const filteredProperties: PropertyListItem[] = useMemo(() => {
    ...
    const base = propertiesArray
      ...
      .filter((p) =>
        filterWithBookingsOnly ? (propertyHasBookingsInViewMonth.get(Number(p.id)) ?? false) : true
      )
  ...
  /**
   * Reservas a serem mostradas/analizadas.
   */
  const filteredBookings: BookingResponse[] = useMemo(() => {
    if (focusPropertie) return bookings.get(Number(focusPropertie.id)) ?? []
    return filteredProperties.flatMap((p) => bookings.get(Number(p.id)) ?? [])
  }, [focusPropertie, bookings, filteredProperties])

  /**
   * Cache de emails para suportar hover rico no calendário.
   */
  const userEmailById = useBookingUserEmails(filteredBookings)

  /**
   * Altera o foco para a propriedade indicada no `TimelineItemWithNames`.
   *
   * - clicar na mesma propriedade alterna (toggle) foco on/off
   */
  ...
  /**
   * Inicializa dados base da dashboard.
   *
   * Nota: mantemos `today` e `isClient` definidos em `useEffect` para evitar
   * diferenças SSR/Client em valores não determinísticos (`new Date()`).
   */
  useEffect(() => {
    setIsClient(true)
    setToday(new Date())
    ...
    const loadData = async () => {
      ...
      await PropertyService.listMine()
        ...
        .then((list: PropertyListItem[]) => {
          for (const p of list) fetchedProperties.set(p.id, p)
        })
        .catch(() => {
          console.error("FAIL FETCH PROPERTY")
        })

      const fetchedBookings: Map<number, BookingResponse[]> = new Map<number, BookingResponse[]>()
      for (const p of fetchedProperties.values()) {
        const books: BookingResponse[] = await BookingService.getBookingsByProperty(p.id).then(
          (br: BookingResponse[]) => br
        )
  ...
  /**
   * Carrega regras de sazonalidade apenas quando existe foco.
   */
  useEffect(() => {
  ...
  /**
   * Projeção (por dia) do multiplicador de sazonalidade para o mês em vista.
   *
   * Só existe quando há foco (1 propriedade).
   */
  const seasonalityMultipliers = useMemo(() => {
    if (!focusPropertie) return null
    return computeSeasonalityMultipliers(
      seasonalityRules,
  ...
  /**
   * Adapta propriedades + reservas para o formato do calendário (timeline items).
   */
  const calendarItems = useMemo(() => {
    return createCalendarItems(filteredProperties, filteredBookings, userEmailById)
  }, [filteredProperties, filteredBookings, userEmailById])
    ...
    <div className={"flex flex-col mx-10 my-5 gap-5"}>
      ...
      <DashboardTimeline
        ...
        seasonalityMultipliers={
          focusPropertie && seasonalityMultipliers && !isLoadingSeasonality ? seasonalityMultipliers : null
        }
      />
