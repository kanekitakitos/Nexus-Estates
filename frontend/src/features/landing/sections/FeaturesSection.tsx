...
export function FeaturesSection({ s }: { s: typeof SECTIONS[0] }) {
  return (
      <div className="relative w-full h-full flex flex-col justify-center pl-12 pr-10 md:pr-44 pt-16 pb-28 overflow-hidden" data-bg-obstacle>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-20 auto-rows-[12rem]" style={{ maxWidth:"calc(100% - 60px)" }}>
          {BENTO_FEATURES.map((f, i) => (
              <motion.div key={f.n}
                          ...
                          transition={{ duration: 0.6, ease: ease.out, delay: 0.3 + i * 0.1 }}
              ...
              <motion.span
                ...
                transition={{ duration: 4.4, repeat: Infinity, ease: ease.inOut, delay: i * 0.18 }}
              >
                {f.n}
              </motion.span>
                </div>
                <div className="relative z-10">
                  ...
                  <p className="text-xs leading-relaxed max-w-[90%]" style={{ color:`${B.black}73` }}>
                    {f.desc}
                  </p>
                </div>
